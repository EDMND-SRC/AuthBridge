  ...
  try {
    ...
    if (!verification) {
      ...
      return {
        ...
        headers: responseHeaders(),
        body: JSON.stringify(
          createErrorResponse('VERIFICATION_NOT_FOUND', 'Verification not found', requestId)
        ),
      };
    }
    ...
    return {
      ...
      headers: responseHeaders(),
      body: JSON.stringify(
        createErrorResponse('INTERNAL_ERROR', 'Failed to get verification status', requestId)
      ),
    };
  }
}
 ...
 */
function mapDocumentStatusToOcrStatus(
  status: string
