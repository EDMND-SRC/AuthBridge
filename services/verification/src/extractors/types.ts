 ...
 */
export type SupportedCountry = 'BW'; // Botswana - expand as needed: 'ZA' | 'NA' | 'ZW' | 'ZM'
...
export interface NationalIdFields extends BaseExtractedFields {
  ...
  placeOfBirth?: string;
  placeOfIssue?: string;
  // Country-specific fields added by implementations
...
export interface DriversLicenceFields extends BaseExtractedFields {
  ...
  nationalIdNumber?: string; // Links to national ID
  licenceClass?: string;
...
export interface PassportFields extends BaseExtractedFields {
  ...
  nationalIdNumber?: string; // Personal number / national ID link
  type?: string; // P = Passport
 ...
 */
export function getExtractorKey(country: SupportedCountry, documentType: DocumentType): string {
