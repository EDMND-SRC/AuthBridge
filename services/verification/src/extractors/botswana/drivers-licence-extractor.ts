...
export class BotswanaDriversLicenceExtractor implements DocumentExtractor<BotswanaDriversLicenceFields> {
  readonly country: SupportedCountry = 'BW';
  readonly documentType: DocumentType = 'drivers_licence';
  ...
  extract(blocks: TextractBlock[]): ExtractionResult<BotswanaDriversLicenceFields> {
    const fields: Partial<BotswanaDriversLicenceFields> = {};
    ...
    const missingRequiredFields = this.requiredFields.filter(
      (field) => !fields[field as keyof BotswanaDriversLicenceFields]
    );
    ...
    return {
      fields: fields as BotswanaDriversLicenceFields,
      confidence,
      overallConfidence,
  ...
  validate(fields: BotswanaDriversLicenceFields): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  private extractName(
    ...
    fields: Partial<BotswanaDriversLicenceFields>,
  ...
  private extractLabeledFields(
    ...
    if (genderMatch) {
      fields.gender = genderMatch[1] as 'M' | 'F';
