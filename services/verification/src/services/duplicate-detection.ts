...
export class DuplicateDetectionService {
  ...
  async checkDuplicates(
    ...
    try {
      ...
      const now = new Date();
      const cases: DuplicateCase[] = duplicateCases.map((c) => ({
      ...
      const sameClientCount = cases.filter(
        (c) => c.clientId === currentClientId
      ).length;
      const crossClientCount = cases.filter(
        (c) => c.clientId !== currentClientId
      ).length;

      const biometricMismatches = cases.filter((c) => {
        if (!c.biometricScore) return false;
      ...
      const statusMismatches = cases.filter(
        (c) => c.status === 'rejected' || c.status === 'auto_rejected'
      ).length;
