...
import type { BiometricData, BiometricSummary } from '../types/biometric';
...
export class BiometricStorageService {
  ...
  async storeBiometricResults(
    ...
    try {
      ...
      await this.dynamoDBService.updateItem({
        ...
        UpdateExpression:
          'SET #biometricData = :biometricData, #status = :status, #processedAt = :processedAt',
   ...
   * @param biometricData Complete biometric verification result
   */
  async updateVerificationWithBiometricSummary(
    verificationId: string,
    biometricData: BiometricData
  ): Promise<void> {
    const now = new Date().toISOString();

    const biometricSummary: BiometricSummary = {
    ...
    try {
      await this.dynamoDBService.updateItem({
        ...
        UpdateExpression:
          'SET #biometricSummary = :biometricSummary, #status = :status, #updatedAt = :updatedAt',
        ExpressionAttributeNames: {
