 ...
 */
export function assessImageQuality(blocks) {
    const issues = [];
    ...
    const avgConfidence = lineBlocks.reduce((sum, b) => sum + (b.Confidence || 0), 0) / lineBlocks.length;
    ...
    const hasOmangIndicators = fullText.includes('OMANG') ||
 ...
 */
export function shouldRequestRecapture(qualityResult, extractedFieldCount, requiredFieldCount) {
