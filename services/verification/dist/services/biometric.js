...
export class BiometricService {
     ...
     */
    async processBiometric(bucketName, livenessSessionId, selfieKey, idPhotoKey) {
        const startTime = Date.now();
        try {
            // Step 1: Validate liveness
            logger.info('Starting liveness detection', { livenessSessionId });
            const livenessResult = await this.rekognitionService.detectFaceLiveness(livenessSessionId);
            // Step 2: Compare faces
            logger.info('Starting face comparison', { selfieKey, idPhotoKey });
            const faceComparisonResult = await this.rekognitionService.compareFaces(bucketName, selfieKey, idPhotoKey);
            // Step 3: Calculate overall score
            const overallScore = this.calculateOverallScore(livenessResult.confidence, faceComparisonResult.similarity);
            // Step 4: Determine if passed and requires manual review
            const passed = livenessResult.passed &&
                faceComparisonResult.passed &&
                overallScore >= OVERALL_THRESHOLD;
            const requiresManualReview = !livenessResult.passed ||
                !faceComparisonResult.passed ||
                overallScore < OVERALL_THRESHOLD ||
     ...
     */
    calculateOverallScore(livenessScore, similarityScore) {
