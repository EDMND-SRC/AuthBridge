...
import { extractOmangFrontFields, extractOmangBackFields, calculateOverallConfidence, } from '../utils/field-extractors';
...
export class OmangOcrService {
     ...
     */
    async extractOmangFront(bucket, key) {
        ...
        const missingFields = REQUIRED_FRONT_FIELDS.filter((field) => !fields[field]);
        // Determine if manual review is required
        const requiresManualReview = overallConfidence < CONFIDENCE_THRESHOLD_LOW ||
     ...
     */
    async extractOmangBack(bucket, key) {
        const startTime = Date.now();
        // Call Textract
        const textractResponse = await this.textractService.detectDocumentTextWithRetry(bucket, key);
        // Extract fields using pattern matching
        ...
        const overallConfidence = calculateOverallConfidence(confidence);
        // Identify missing fields
        const missingFields = REQUIRED_BACK_FIELDS.filter((field) => !fields[field]);
        // Determine if manual review is required
...
//# sourceMappingURL=omang-ocr.js.map