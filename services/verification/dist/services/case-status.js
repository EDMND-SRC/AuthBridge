...
export class CaseStatusService {
    webhookService;
    dynamoDBService;
    constructor(webhookService, dynamoDBService) {
     ...
     */
    async updateStatus(verificationId, newStatus, additionalData) {
        // Load current case
        const verificationCase = await this.dynamoDBService.getVerification(verificationId);
        if (!verificationCase) {
