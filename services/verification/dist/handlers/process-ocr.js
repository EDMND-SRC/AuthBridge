    ...
    for (const record of event.Records) {
        try {
            ...
            if (message.documentType !== 'selfie' && ocrResult.rawTextractResponse) {
                const blocks = (ocrResult.rawTextractResponse.Blocks || []);
                ...
                if (shouldRequestRecapture(qualityResult, extractedFieldCount, requiredFields)) {
                    ...
                    await notifyPoorQualityImage(message.verificationId, message.documentId, qualityResult.qualityScore, qualityResult.issues);
                }
            }
            ...
            if (message.documentType === 'omang_front' &&
                ocrResult.extractedFields.idNumber &&
                ocrResult.extractedFields.dateOfExpiry) {
                validationResult = validationService.validate({
                    idNumber: ocrResult.extractedFields.idNumber,
                    dateOfExpiry: ocrResult.extractedFields.dateOfExpiry,
                });
                // Record validation metrics
                await recordOmangValidationMetrics(validationResult.overall.valid, message.documentType, validationResult.overall.errors, validationResult.overall.warnings);
            ...
            await storageService.storeOcrResults(message.verificationId, message.documentId, message.documentType, ocrResult, qualityResult, validationResult);
            // Update verification case with extracted customer data
            await storageService.updateVerificationWithExtractedData(message.verificationId, ocrResult);
            const durationMs = Date.now() - startTime;
            // Record metrics
            await recordOcrMetrics(true, durationMs, ocrResult.confidence.overall, message.documentType, ocrResult.requiresManualReview);
...
//# sourceMappingURL=process-ocr.js.map