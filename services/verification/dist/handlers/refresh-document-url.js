    ...
    try {
        ...
        if (!verificationId || !documentId) {
            return {
                statusCode: 400,
                headers: corsHeaders(),
                body: JSON.stringify(createErrorResponse('VALIDATION_ERROR', 'Verification ID and Document ID are required', requestId)),
            };
        }
        ...
        if (!document) {
            ...
            return {
                statusCode: 404,
                headers: corsHeaders(),
                body: JSON.stringify(createErrorResponse('NOT_FOUND', 'Document not found', requestId)),
            };
        }
        // Generate new presigned URL
        const { url: presignedUrl, expiresAt: presignedUrlExpiresAt } = await s3.generatePresignedUrl(document.s3Key);
        ...
        return {
            statusCode: 500,
            headers: corsHeaders(),
            body: JSON.stringify(createErrorResponse('INTERNAL_ERROR', 'Failed to refresh document URL', requestId)),
        };
    }
}
...
//# sourceMappingURL=refresh-document-url.js.map